...
gmm.fit(data)
print(np.round(gmm.predict_proba(data)))

i =  0 #test example, give any number
data = np.zeros((1,14))
for j in range(14):
    data[0,j]=np.std(new_a[i][:,j])
data_new = np.zeros((1,2))
...
data_new[0,1] =  data[0,6] + data[0,13]
data = data_new
