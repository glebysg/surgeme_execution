...
for iname, cname in zip(images, cloud):
    ...
    print("min_x",np.min(cloudp[:,3]))
    print("min_y",np.min(cloudp[:,4]))
    print("max_x",np.max(cloudp[:,3]))
    print("max_y",np.max(cloudp[:,4]))
    ...
    # for i in range(y_grid):
        # for j in range(x_grid):
            # objp[i,j,1] = i*2
            # objp[i,j,0] = j*2
    # # Divide all the points by 1000 so they are in meters
    # objp = objp/100.0
